        ...
        <div>
            ...
            >
                <div 
                    ...
                    on:scroll=on_scroll
                >
                    <div style=move || format!("height: {}px; position: relative;", total_height())>
                        {move || {
                            let (start, end) = visible_window.get();
                            
                            tasks[start..end].iter().enumerate().map(|(i, task)| {
                                ...
                                view! {
                                    <div
                                        ...
                                        data-id=format!("task-{}", task.id.as_ref().map(|id| id.inner()).unwrap_or_default())
                                    >
                                        <TaskItem
                                            task=task.clone()
                                            on_toggle=on_toggle
